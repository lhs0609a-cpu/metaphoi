...
async def complete_test_session(
    ...
    # Update session with fraud score if provided
    session_update = {
        "status": "completed",
    ...
    if fraud_score is not None:
        session_update["fraud_score"] = fraud_score

    supabase.table("test_sessions").update(session_update).eq("id", session_id).execute()

    # Create test result with interpretation
    result_data = {
        "session_id": session_id,
    ...
    if interpretation:
        result_data["processed_result"] = interpretation

    result = supabase.table("test_results").insert(result_data).execute()
